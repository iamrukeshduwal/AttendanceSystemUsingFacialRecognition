...
class Face_Recognition:
    ...
    def __init__(self,root):
        ...
        std_b1_2 = Button(bg_img,command=self.drop_dup,text="Drop Duplicate",cursor="hand2",font=("tahoma",15,"bold"),bg="white",fg="navyblue")
        std_b1_2.place(x=600,y=450,width=180,height=45)
    #=====================Attendance===================
    def drop_dup(self):
        f=pd.read_csv("attendance.csv")
        Dup_Rows = f[f.duplicated()]
        DF_RM_DUP = f.drop_duplicates(subset=['ID','Roll',"Name","Date"])
    ...
    def mark_attendance(self,i,r,n):
        with open("attendance.csv","r+",newline="\n") as f:
            ...
            if((i not in name_list)) or ((r not in name_list)) or ((n not in name_list)):
                now=datetime.now()
